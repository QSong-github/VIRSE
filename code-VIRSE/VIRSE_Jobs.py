...
"""
Execute one VIRSE run for a given K and write output plots/files.
"""
import os
import VIRSE_Files
import VIRSE_Plots
from VIRSE_Algorithm import Run_EM
import numpy as np
...
def Run_EMJob(X, bvfile_basename, MIN_ITS, INFO_THRESH, CONV_CUTOFF,
    ...
    if K == 1:
        ...
        VIRSE_Plots.NumReads_File(bvfile_basename, X, outplot_dir)

    attempt = 1
    while True:
        print(f"[VIRSE] ▶ Starting attempt {attempt} for K={K}, run={run}")
